...
function VerifyOtpContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setMessage("Verification Successful! Login you in...");
        if (result.user) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[#F3F4F6] p-4">
      <div className="bg-white rounded-[32px] shadow-[0_24px_60px_-12px_rgba(0,0,0,0.1)] p-10 w-full max-w-[480px]">
        {/* Icon */}
          ...
          <div className="flex justify-center gap-2 sm:gap-3">
            {otp.map((digit, index) => (
              <input
                ...
                className="w-12 h-14 sm:w-14 sm:h-16 border-2 border-gray-200 rounded-xl text-center text-2xl font-bold text-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all bg-gray-50 focus:bg-white"
              />
            ))}
          </div>
        ...
        <div className="text-center mt-8">
          <p className="text-sm text-gray-500">
            ...
            <button
              ...
              onClick={() => alert("Resend logic needs implementation")}
            >
              Resend
            </button>
          </p>
